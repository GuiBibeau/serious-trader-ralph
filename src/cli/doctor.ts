...
export async function runDoctor(config: SolmoltConfig): Promise<void> {
  ...
  if (!config.wallet.privateKey && !config.wallet.keyfilePath) {
    results.push({ check: 'wallet.present', ok: false, detail: 'missing privateKey or keyfilePath' });
  } else {
    results.push({ check: 'wallet.present', ok: true });
  }

  const missing = ['gateway.authToken', 'jupiter.apiKey', 'llm.apiKey'].filter((k) => !getConfigValue(config, k));
  if (missing.length > 0) {
    results.push({ check: 'secrets.present', ok: false, detail: `missing ${missing.join(', ')}` });
  } else {
...
async function checkRpc(endpoint: string): Promise<{ ok: boolean; detail?: string }> {
  try {
    const payload = {
      jsonrpc: '2.0',
      id: 1,
