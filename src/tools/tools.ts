...
export function registerDefaultTools(registry: ToolRegistry, jupiter: JupiterClient): void {
      ...
      parameters: {
        ...
        properties: {
          quoteSummary: {
            ...
            required: ['inAmount', 'outAmount', 'priceImpactPct', 'routeLabels'],
          ...
          policySnapshot: {
            ...
            required: ['killSwitch', 'allowedMints', 'maxTradeAmountLamports', 'maxSlippageBps', 'maxPriceImpactPct', 'cooldownSeconds'],
            additionalProperties: true,
          },
        },
  ...
  registry.register({
    ...
    execute: async (ctx: ToolContext, input: { content: string; triggerTick?: boolean }) => {
      if (!ctx.agentControl) {
...
function enforcePolicy(policy: PolicySnapshot, quoteResponse: Record<string, unknown>): void {
  if (policy.killSwitch) {
    throw new Error('kill-switch-enabled');
  }
  const inputMint = typeof quoteResponse['inputMint'] === 'string' ? quoteResponse['inputMint'] : undefined;
  ...
  if (policy.allowedMints.length > 0) {
    if ((inputMint && !policy.allowedMints.includes(inputMint)) || (outputMint && !policy.allowedMints.includes(outputMint))) {
      throw new Error('mint-not-allowed');
    }
  }
  const inAmount = typeof quoteResponse['inAmount'] === 'string' ? quoteResponse['inAmount'] : undefined;
  ...
  if (typeof priceImpactPctRaw === 'number' && priceImpactPctRaw > policy.maxPriceImpactPct) {
