...
function collectJupiterLogHints(logMessages: string[]): string[] {
  const hints: string[] = [];
  for (const line of logMessages) {
    const lower = line.toLowerCase();
    if (lower.includes("jupiter") || lower.includes("route")) {
      hints.push(line);
      ...
      return [
        {
          ...
          meta: {
            ...
            logHints: collectJupiterLogHints(context.logMessages),
          },
        },
      ];
    },
