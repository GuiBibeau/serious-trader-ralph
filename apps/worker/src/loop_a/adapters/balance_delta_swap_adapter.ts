...
type SupportedVenueMarketType = "spot" | "clob";

type BalanceDeltaSwapAdapterConfig = {
  id: string;
  protocol: string;
  venue: string;
  marketType: SupportedVenueMarketType;
  programIds: string[];
...
function collectLogHints(
  ...
  for (const line of logMessages) {
    ...
    if (
      normalizedPatterns.some((pattern) => lower.includes(pattern)) ||
      lower.includes(fallback.toLowerCase())
    ) {
      hints.push(line);
...
export function createBalanceDeltaSwapAdapter(
  config: BalanceDeltaSwapAdapterConfig,
): ProtocolAdapter {
  return {
    id: config.id,
    ...
    decode(context: DecodingContext): ProtocolEvent[] {
      ...
      const identifier =
        ...
        (instruction?.accounts[config.identifierAccountIndex ?? 0] ?? null);
      const logHints = collectLogHints(
        context.logMessages,
        config.logHintPatterns ?? [],
        config.venue,
      );
      ...
      return [
        {
          ...
          meta: {
            ...
            ...(logHints.length > 0 ? { logHints } : {}),
            ...(config.identifierKind === "pool" &&
            typeof identifier === "string"
              ? { pool: identifier }
              : {}),
            ...(config.identifierKind === "market" &&
            typeof identifier === "string"
              ? { market: identifier }
              : {}),
          },
