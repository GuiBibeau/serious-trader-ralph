...
describe("worker runtime deployment evaluate route", () => {
  test("requires admin auth", async () => {
    const { env, sqlite } = createOpsEnv();
    try {
      const response = await worker.fetch(
        new Request(
          "http://localhost/api/admin/ops/runtime/deployments/deployment_123/evaluate",
          {
            method: "POST",
            headers: {
  ...
  test("evaluates a deployment through the admin route", async () => {
    const { env, sqlite } = createOpsEnv();
    try {
      const response = await worker.fetch(
        new Request(
