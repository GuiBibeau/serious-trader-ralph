...
const stubConfig: SolmoltConfig = {
  rpc: { endpoint: 'http://localhost:8899' },
  wallet: {},
  jupiter: { apiKey: 'test', baseUrl: 'https://api.jup.ag' },
  solana: { sdkMode: 'web3' },
  llm: { provider: 'openai_chat', baseUrl: 'https://api.z.ai/api/paas/v4', apiKey: 'test', model: 'glm-4.7' },
...
const stubSolana: SolanaAdapter = {
  ...
  getLatestBlockhash: async () => ({ blockhash: '111', lastValidBlockHeight: 0 }),
...
test('invalid quote args are rejected before execution', async () => {
  ...
  const jupiter = new JupiterClient(stubConfig.jupiter.baseUrl, stubConfig.jupiter.apiKey);
  ...
  await expect(
    registry.invoke('market.jupiter_quote', ctx, { inputMint: '', outputMint: '', amount: 'abc', slippageBps: -1 })
