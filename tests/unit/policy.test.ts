...
test('kill switch blocks trade', () => {
  const result = evaluateTrade({ ...basePolicy, killSwitch: true }, baseQuote, baseBalances);
  expect(result.allow).toBe(false);
  expect(result.reasons).toContain('kill-switch-enabled');
});

test('price impact block', () => {
  const result = evaluateTrade({ ...basePolicy, maxPriceImpactPct: 0.01 }, baseQuote, baseBalances);
  expect(result.allow).toBe(false);
...
test('mint allowlist enforced', () => {
  ...
  const result = evaluateTrade({ ...basePolicy, allowedMints: ['OTHER'] }, baseQuote, balances);
  expect(result.allow).toBe(false);
