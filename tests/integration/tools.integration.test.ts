...
import type { SolmoltConfig } from '../../src/config/config.js';
...
function buildConfig(): SolmoltConfig {
  return {
    rpc: {
...
integrationTest('market.jupiter_quote (integration)', async () => {
  const { registry, ctx } = setup();
  const inputMint = process.env.QUOTE_INPUT_MINT || 'So11111111111111111111111111111111111111112';
  const outputMint = process.env.QUOTE_OUTPUT_MINT || 'EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v';
...
swapTest('swap simulation (build + sign + simulate)', async () => {
  ...
  if (!config.wallet.privateKey && !config.wallet.keyfilePath) {
    throw new Error('WALLET_PRIVATE_KEY or WALLET_KEYFILE required for swap simulation');
  }
  ...
  if (requestAirdrop) {
    const secretKey = loadSecretKey(config.wallet.privateKey, config.wallet.keyfilePath);
  ...
  const inputMint = process.env.SWAP_INPUT_MINT || 'So11111111111111111111111111111111111111112';
  const outputMint = process.env.SWAP_OUTPUT_MINT || 'EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v';
  ...
  const simulation = (await ctx.solana.simulateRawTx(signed)) as { err?: unknown; logs?: string[] | null } | undefined;
